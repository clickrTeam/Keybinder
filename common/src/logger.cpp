...
QString Logger::getBackupFilePath() {
    ...
    QString backupFilePath = LOG_DIR + "/" + LOG_FILE_NAME + "_" + timestamp + ".bak";
    return backupFilePath;
}

void Logger::removeOldestBackupFile() {
    QDir dir(LOG_DIR);
    QStringList backupFiles = dir.entryList(QStringList(LOG_FILE_NAME + "_*.bak"));
...
void myMessageHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
    ...
    .arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss"))
        .arg(type == QtDebugMsg ? "DEBUG" : type == QtInfoMsg ? "INFO" : type == QtWarningMsg ? "WARNING" : type == QtCriticalMsg ? "CRITICAL" : "FATAL")
        .arg(msg);
    Logger logger;
