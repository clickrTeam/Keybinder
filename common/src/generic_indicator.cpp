...
GenericIndicator::GenericIndicator(const QString &note,
    ...
    label->setStyleSheet("QLabel { color: white; background-color: rgba(0, 0, "
                         "0, 180); padding: 10px; "
    ...
    // Find the screen under the cursor, fallback to a screen containing the
    // cursor
...
void GenericIndicator::place_in_position(IndicatorPosition location,
                                         QScreen *screen) {
    if (screen) {
        // Compute global target coordinates using availableGeometry() (logical
        // pixels)
        ...
        if (!windowHandle()) {
            createWinId(); // forces native handle creation on platforms where
                           // needed
        }
        ...
        // Move the widget in global coordinates before showing to avoid
        // compositor placement
        move(x, y);
        show();
    } else {
        qCritical() << "Attempted to calculate indicator position on a null "
                       "pointer screen.";
    }
}
