...
int main()
{
    ...
    // Re-open keyb now that we know proper device. It must be reopened because of the library.
    ...
    while(!escape_pressed)
    {
        ...
        {
            if (event.type == EV_KEY &&  event.value == 1) // ESC has been pressed
            {
                cout << "User pressed key: " << libevdev_event_code_get_name(EV_KEY, event.code) << endl;
